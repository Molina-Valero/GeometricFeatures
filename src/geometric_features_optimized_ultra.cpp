    ...
    std::vector<Vec3<double>>& points_buffer)
{
  ...
  if (lambda_sum > epsilon) {
    ...
    features.linearity = (lambda1 - lambda2) * inv_sum;
    features.planarity = (lambda2 - lambda3) * inv_sum;
    features.sphericity = lambda3 * inv_sum;
    features.anisotropy = (lambda1 - lambda3) * inv_sum;
    features.surface_variation = lambda3 * inv_sum;
  }
...
// [[Rcpp::export]]
Rcpp::DataFrame geometric_features_batch(
